...
컨테이너 런타임(Docker, CRI-O 등)은 컨테이너의 표준 출력(``STDOUT``)과 표준 에러(``STDERR``)로 나가는 내용을 별도의 로깅 에이전트 없이도 자동으로 수집합니다. 그래서 OpenShift는 컨테이너가 모든 정보를 ``STDOUT``에 로그로 남기기를 기대합니다. 이 방식을 따르면
...
[.bordershadow]
image::parksmap-view-logs-link.png[Resources 탭에서 Pod 옆에 표시된 View Logs 링크,link="self",window=_blank]

**View Logs** 링크를 클릭하여 **Pod**의 로그를 확인합니다:

[.bordershadow]
image::parksmap-logging-console-logs.png[웹 콘솔에서 확인한 parksmap Pod의 컨테이너 로그,link="self",window=_blank]
